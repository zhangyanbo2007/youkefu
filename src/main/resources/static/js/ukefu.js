var layer , laydate, iframe , layerwin , cursession , acceptCallOut = true , loaddingtip = "<div style='height: 100%;width: 100%;display: flex;justify-content: center;align-items: center;'><i style='color:#2a304d;font-size:60px;' class='layui-icon layui-icon-loading-1 layui-icon layui-anim layui-anim-rotate layui-anim-loop'></i></div>";
layui.extend({
...
$(document).ready(function(){
	...
	$(document).on('click','body', function ( e ) {
		if($(e.target).closest(".layui-layer-page").length == 0 && top.$('#mainajaxwin').length > 0 && $(e.target).closest(".layui-layer-shade").length == 0){
			top.layer.closeAll();
		}
	});
	...
	$(document).on('click','.iframe_btn', function(){
		var type = $(this).data('type');
		if(type == "tabAdd"){
			active.tabAdd($(this).data('href') , $(this).data('title'), $(this).data('id'));
		}else if(type == "tabChange"){
			active.tabChange($(this).data('href') , $(this).data('title'), $(this).data('id') ,$(this).data('top'));
		}
		$(".layui-nav-item.layui-this,.ukefu-left-menu.layui-this").removeClass("layui-this");
		$(this).closest(".ukefu-left-menu").addClass("layui-this");
		$(this).closest(".layui-nav-item").addClass("layui-this");
	});
	$(document).on('click','[data-toggle="ajax"]', function ( e ) {
		...
		if(url && url != "javascript:void(0)"){
			...
			var fadein = $(this).attr("data-type");
			var target = $(this) ;
			top.iframe = window.frameElement && window.frameElement.id || '';
			if(top.$('#mainajaxwin').length > 0){
				top.$('#mainajaxwin').empty().append(loaddingtip);
				$.ajax({
					url:url,
					cache:false,
					success: function(data){
						top.$('#mainajaxwin').empty().append(data);
					}
				});
			}else{
...
var active = {
	...
	tabChange : function(href, title, id ,top) {
		var layelement = layui.element;
		...
		$(".layui-this").each(function() {
			if ($(this).hasClass("ukefu-top-menu")) {
